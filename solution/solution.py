...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        # 1. Mark each chunk relevant if it covers >= threshold of expected.
        ...
        # 2 + 3. Rank-aware Average Precision.
        running_relevant = 0
        ap_sum = 0.0
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial judge scoring an AI response.\n"
            f"Question:\n{question}\n\n"
            f"Answer to score:\n{answer}\n\n"
            "Rubric criteria (score each from 0.0 to 1.0):\n"
            f"{rubric_lines}\n\n"
            "Respond with ONLY a JSON object mapping each criterion to a score "
            'between 0.0 and 1.0, e.g. {"accuracy": 0.8}.'
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        # Flatten all individual criterion scores across the batch.
        all_scores: list[float] = []
        for entry in scores_batch:
            criterion_scores = entry.get("scores", {})
            all_scores.extend(criterion_scores.values())

        avg = sum(all_scores) / len(all_scores) if all_scores else 0.0

        # Positional bias: first response in each pair consistently scores
        # higher than the rest. With a single-item batch we cannot establish a
        # pattern, so default to False.
        positional_bias = False
        if len(scores_batch) >= 2:
            first_avgs: list[float] = []
            rest_avgs: list[float] = []
            for entry in scores_batch:
                vals = list(entry.get("scores", {}).values())
                if not vals:
                    continue
                mean = sum(vals) / len(vals)
                first_avgs.append(mean)
            # Treat the first scored response as the "first position".
            if len(first_avgs) >= 2:
                first = first_avgs[0]
                rest = first_avgs[1:]
                rest_mean = sum(rest) / len(rest)
                positional_bias = first > rest_mean + 0.2
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest_value = scores[lowest_metric]

        # If several metrics are all low, the problem is systemic.
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            analyzer_lines.append(
                f"| {fid} | {ftype} | {root_cause} | {fix} | Open |"
            )
        return "\n".join(analyzer_lines)
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            "hallucination": "Implement a hallucination checker to filter unsupported "
                             "claims and tighten the faithfulness guardrail",
            "irrelevant": "Improve prompt clarity and intent routing so answers "
                          "address the actual question",
            "incomplete": "Add few-shot examples showing complete answers and "
                          "increase the retrieval context window",
            "off_topic": "Strengthen intent detection / query routing to keep "
                         "answers on topic",
            "refusal": "Relax over-strict guardrails so valid questions are "
                       "answered instead of refused",
        }

        # Prioritize by frequency (most common failure type first).
        for ftype, _count in sorted(
            categories.items(), key=lambda kv: kv[1], reverse=True
        ):
            if ftype in suggestion_map:
                suggestions.append(suggestion_map[ftype])

        # Generic backstops to guarantee at least 3 actionable suggestions.
        generic = [
            "Increase chunk size / overlap in the RAG pipeline to reduce "
            "context fragmentation",
            "Expand the golden dataset with the observed failure cases for "
            "regression coverage",
            "Add an LLM-as-Judge calibration pass against human labels to catch "
            "scoring bias",
        ]
        for g in generic:
